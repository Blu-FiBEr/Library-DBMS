#include <stdio.h>
#include <stdlib.h>
#include <string.h>

#include "libsys.h"

#define TREPORT(a1, a2)                  \
	printf("Status %s: %s\n\n", a1, a2); \
	fflush(stdout);

void process_line(char *test_case);

int main(int argc, char *argv[])
{
	FILE *cfptr;
	char test_case[50];

	if (argc != 2)
	{
		fprintf(stderr, "Usage: %s testcasefile\n", argv[0]);
		exit(1);
	}

	cfptr = (FILE *)fopen(argv[1], "r");
	while (fgets(test_case, sizeof(test_case) - 1, cfptr))
	{
		// printf("line:%s",test_case);
		if (!strcmp(test_case, "\n") || !strcmp(test_case, ""))
			continue;
		process_line(test_case);
	}
	return 0;
}

void process_line(char *test_case)
{
	// Initialize Valriable
	// Handle testcases for
	// CREATE
	// OPEN
	// ADD_BOOK
	// ADD_STUDENT
	// SEARCH_BOOK_ISBN
	// SERACH_BOOK_TITLE
	// SEARCH_STUDENT_NAME
	// DELETE_STUDENT
	// DELETE_BOOK
	// ISSUE

	char book_name[30] = "book_demo", stud_name[30] = "stud_demo", issue_name[30] = "issue";
	char command[30], param1[10], param2[10], info[1000];
	char title[40], name[40];
	int isbn, status, rec_size, expected_status;
	struct Book testBook;

	int rollno, rec_size2;
	struct Student testStud;

	strcpy(testBook.title, "YOGI");
	strcpy(testBook.author, "yoganand");
	testBook.price = 100.50;

	strcpy(testStud.name, "ABC");
	strcpy(testStud.address, "Bangalore");
	testStud.cgpa = 3.00;

	rec_size = sizeof(struct Book);
	rec_size2 = sizeof(struct Student);

	sscanf(test_case, "%s%s%s", command, param1, param2);
	printf("Test case: %s", test_case);
	fflush(stdout);
	if (!strcmp(command, "CREATE"))
	{
		if (!strcmp(param2, "0"))
			expected_status = BOOK_SUCCESS;
		else
			expected_status = BOOK_FAILURE;

		status = libsys_create(book_name, stud_name, issue_name);
		if (status == expected_status)
		{
			TREPORT("PASS", "");
		}
		else
		{
			sprintf(info, "libsys_open returned status %d", status);
			TREPORT("FAIL", info);
		}
	}

	else if (!strcmp(command, "OPEN"))
	{
		if (!strcmp(param2, "0"))
			expected_status = BOOK_SUCCESS;
		else
			expected_status = BOOK_FAILURE;

		status = libsys_open(book_name, stud_name, issue_name);
		if (status == LIB_SUCCESS)
			status = BOOK_SUCCESS;
		else
			status = BOOK_FAILURE;
		if (status == expected_status)
		{
			TREPORT("PASS", "");
		}
		else
		{
			sprintf(info, "libsys_open returned status %d", status);
			TREPORT("FAIL", info);
		}
	}
	else if (!strcmp(command, "ADD_BOOK"))
	{
		if (!strcmp(param2, "0"))
			expected_status = BOOK_SUCCESS;
		else
			expected_status = BOOK_FAILURE;

		sscanf(param1, "%d", &isbn);
		testBook.isbn = isbn;
		status = put_book_by_isbn(isbn, &testBook);
		if (status == LIB_SUCCESS)
			status = BOOK_SUCCESS;
		else
			status = BOOK_FAILURE;
		if (status == expected_status)
		{
			TREPORT("PASS", "");
		}
		else
		{
			sprintf(info, "put_book_by_isbn returned status %d", status);
			TREPORT("FAIL", info);
		}
	}
	else if (!strcmp(command, "ADD_STUDENT"))
	{
		if (!strcmp(param2, "0"))
			expected_status = 0;
		else
			expected_status = 1;

		sscanf(param1, "%d", &rollno);
		testStud.rollno = rollno;
		status = put_student_by_rollno(rollno, &testStud);
		if (status == LIB_SUCCESS)
			status = 0;
		else
			status = 1;
		if (status == expected_status)
		{
			TREPORT("PASS", "");
		}
		else
		{
			sprintf(info, "put_student_by_rollno returned status %d", status);
			TREPORT("FAIL", info);
		}
	}
	else if (!strcmp(command, "SEARCH_BOOK_ISBN"))
	{
		if (!strcmp(param2, "0"))
			expected_status = BOOK_SUCCESS;
		else
			expected_status = BOOK_FAILURE;

		sscanf(param1, "%d", &isbn);
		testBook.isbn = -1;
		status = get_book_by_isbn(isbn, &testBook);
		if (status == LIB_SUCCESS)
			status = BOOK_SUCCESS;
		else
			status = BOOK_FAILURE;
		if (status != expected_status)
		{
			sprintf(info, "search key: %d; Got status %d", isbn, status);
			TREPORT("FAIL", info);
		}
		else
		{
			// Check if the retrieved values match with what was stored earlier
			if (expected_status == 0)
			{
				if (testBook.isbn == isbn &&
					strcmp(testBook.title, "YOGI") == 0 &&
					strcmp(testBook.author, "yoganand") == 0 &&
					testBook.price == 100.50)
				{
					TREPORT("PASS", "");
				}
				else
				{
					sprintf(info, "Book data not matching... Expected:{%d,%s,%s,%f} Got:{%d,%s,%s,%f}\n",
							isbn, "YOGI", "yoganand", 100.50,
							testBook.isbn, testBook.title, testBook.author, testBook.price);
					TREPORT("FAIL", info);
				}
			}
			else
				TREPORT("PASS", "");
		}
	}

	else if (!strcmp(command, "SEARCH_BOOK_TITLE"))
	{
		if (!strcmp(param2, "0"))
			expected_status = BOOK_SUCCESS;
		else
			expected_status = BOOK_FAILURE;

		sscanf(param1, "%s", title);
		status = get_book_by_title(title, &testBook);
		if (status == LIB_SUCCESS)
			status = BOOK_SUCCESS;
		else
			status = BOOK_FAILURE;
		if (status != expected_status)
		{
			sprintf(info, "search key: %s; Got status %d", title, status);
			TREPORT("FAIL", info);
		}
		else
		{
			// Check if the retrieved values match with what was stored earlier
			if (expected_status == 0)
			{
				if (testBook.isbn == 10000 &&
					strcmp(testBook.title, title) == 0 &&
					strcmp(testBook.author, "yoganand") == 0 &&
					testBook.price == 100.50)
				{
					TREPORT("PASS", "");
				}
				else
				{
					sprintf(info, "Book data not matching... Expected:{%d,%s,%s,%f} Got:{%d,%s,%s,%f}\n",
							10000, title, "yoganand", 100.50,
							testBook.isbn, testBook.title, testBook.author, testBook.price);
					TREPORT("FAIL", info);
				}
			}
			else
				TREPORT("PASS", "");
		}
	}

	else if (!strcmp(command, "SEARCH_STUDENT_NAME"))
	{
		if (!strcmp(param2, "0"))
			expected_status = 0;
		else
			expected_status = 1;

		sscanf(param1, "%s", name);
		status = get_student_by_name(name, &testStud);
		if (status == LIB_SUCCESS)
			status = 0;
		else
			status = 1;
		if (status != expected_status)
		{
			sprintf(info, "search key: %s; Got status %d", name, status);
			TREPORT("FAIL", info);
		}
		else
		{
			// Check if the retrieved values match with what was stored earlier
			if (expected_status == 0)
			{
				if (testStud.rollno == 1 &&
					strcmp(testStud.name, name) == 0 &&
					strcmp(testStud.address, "Bangalore") == 0 &&
					testStud.cgpa == 3.00)
				{
					TREPORT("PASS", "");
				}
				else
				{
					sprintf(info, "Student data not matching... Expected:{%d,%s,%s,%f} Got:{%d,%s,%s,%f}\n",
							1, name, "Bangalore", 3.00,
							testStud.rollno, testStud.name, testStud.address, testStud.cgpa);
					TREPORT("FAIL", info);
				}
			}
			else
				TREPORT("PASS", "");
		}
	}

	else if (!strcmp(command, "DELETE_BOOK"))
	{
		if (!strcmp(param2, "0"))
			expected_status = BOOK_SUCCESS;
		else
			expected_status = BOOK_FAILURE;

		sscanf(param1, "%d", &isbn);
		testBook.isbn = isbn;
		status = delete_book_by_isbn(isbn);
		if (status == LIB_SUCCESS)
			status = BOOK_SUCCESS;
		else
			status = BOOK_FAILURE;
		if (status == expected_status)
		{
			TREPORT("PASS", "");
		}
		else
		{
			sprintf(info, "delete_book_by_isbn returned status %d", status);
			TREPORT("FAIL", info);
		}
	}
	else if (!strcmp(command, "DELETE_STUDENT"))
	{
		if (!strcmp(param2, "0"))
			expected_status = BOOK_SUCCESS;
		else
			expected_status = BOOK_FAILURE;

		sscanf(param1, "%d", &isbn);
		testStud.rollno = rollno;
		status = delete_student_by_rollno(rollno);
		if (status == LIB_SUCCESS)
			status = BOOK_SUCCESS;
		else
			status = BOOK_FAILURE;
		if (status == expected_status)
		{
			TREPORT("PASS", "");
		}
		else
		{
			sprintf(info, "delete_book_by_isbn returned status %d", status);
			TREPORT("FAIL", info);
		}
	}
	else if (!strcmp(command, "CLOSE"))
	{
		if (!strcmp(param1, "0"))
			expected_status = BOOK_SUCCESS;
		else
			expected_status = BOOK_FAILURE;

		status = libsys_close();
		if (status == LIB_SUCCESS)
			status = BOOK_SUCCESS;
		else
			status = BOOK_FAILURE;
		if (status == expected_status)
		{
			TREPORT("PASS", "");
		}
		else
		{
			sprintf(info, "libsys_close returned status %d", status);
			TREPORT("FAIL", info);
		}
	}

	else if (!strcmp(command, "ISSUE"))
	{
		char param3[40];
		sscanf(test_case, "%s%s%s%s", command, param1, param2, param3);
		if (!strcmp(param3, "0"))
			expected_status = 0;
		else
			expected_status = 1;

		sscanf(param1, "%d", &isbn);
		sscanf(param2, "%d", &rollno);
		status = issue(rollno, isbn);
		if (status == LIB_SUCCESS)
			status = 0;
		else
			status = 1;
		if (status == expected_status)
		{
			TREPORT("PASS", "");
		}
		else
		{
			sprintf(info, "issue returned status %d", status);
			TREPORT("FAIL", info);
		}
	}
}
